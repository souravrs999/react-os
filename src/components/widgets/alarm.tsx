...
export default function AlarmClockWidget({
  alarms = [
  ...
  return (
    <div
      className={cn(
        "group w-full h-full rounded-3xl border bg-background p-4 border-border/20"
      )}
    >
        ...
        {alarms.map((alarm, index) => (
          <div className="flex items-center justify-between" key={index}>
            ...
            <Switch
              ...
              onCheckedChange={(value) => handleToggleChange(value, index)}
            />
          </div>
        ))}
      </div>
